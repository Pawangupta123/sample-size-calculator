    ...
    <LineChart data={data}>
      ...
      {series.map((s, i) => (
        <Line key={s} type={config.smooth ? 'monotone' : 'linear'} dataKey={s}
          ...
          dot={config.showDots ? { r: 4, fill: colors[i] } : false} activeDot={{ r: 6 }}>
...
export function KaplanMeier({ data, config, series, colors }: Props) {
  return (
    <LineChart data={data}>
      ...
      <YAxis domain={[0, 1]} tickFormatter={(v) => `${(v * 100).toFixed(0)}%`} {...axisStyle(config.fontSize)}
      ...
      {config.showLegend && <Legend wrapperStyle={{ fontSize: config.fontSize - 2 }} />}
      {series.map((s, i) => (
        <Line key={s} type="stepAfter" dataKey={s} stroke={colors[i]} strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} />
      ))}
    </LineChart>
  )
}
