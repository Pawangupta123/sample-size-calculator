...
export function buildPubmedQuery(filters: SearchFilters): string {
  ...
  if (filters.openAccessOnly) {
    parts.push('AND free full text[sb]')
  }
...
export function buildEuropePmcQuery(filters: SearchFilters): string {
  ...
  if (filters.studyType === 'rct') {
    parts.push('AND PUB_TYPE:"Randomized Controlled Trial"')
  }
  if (filters.studyType === 'meta') {
    parts.push('AND PUB_TYPE:"Meta-Analysis"')
  }
  if (filters.studyType === 'review') {
    parts.push('AND PUB_TYPE:"Review"')
  }

  if (filters.openAccessOnly) {
    parts.push('AND OPEN_ACCESS:y')
  }
